...
class StockMonitor:
    ...
    def check_stock(self):
        try:
            url = self.config["monitoring"]["stock_api_url"]
            response = requests.get(url, timeout=10)
            response.raise_for_status()
            soup = BeautifulSoup(response.text, 'html.parser')
            stock_indicator = soup.select_one(".game-shop-item .stock-status")  # Adjust selector
            in_stock = stock_indicator and "in stock" in stock_indicator.text.lower()
        ...
        while True:
            if self.check_stock():
                ...
                for account in self.accounts:
                    ...
                    sleep(15)  # Increased delay for browser automation
            else:
